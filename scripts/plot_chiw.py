...
sum_aw = np.loadtxt("{}/aw.dat".format(args.dir))
...
plt.plot(sum_aw[:, 0], sum_aw[:, 1], ls="--", lw=2, label=r'$ A(\omega) $')
for i in range(args.n):
    ...
    plt.plot(chi[:, 0], chi[:, i + 1], color=fc[i], label=label)
    ax.fill(chi[:, 0], chi[:, i + 1], color=fc[i], alpha=0.25)
